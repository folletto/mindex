...
describe.each(JOURNAL_MODES)('multi-process writes (journal_mode = %s)', (journalMode) => {
  it(
    'survives eight processes hammering the same library',
    async () => {
      ...
      try {
        ...
        const duplicates = reopened.db
          .prepare(
            `SELECT slug, COUNT(*) AS n FROM items WHERE deleted_at IS NULL GROUP BY slug HAVING n > 1`,
          )
          .all();
  ...
  it(
    'lets exactly one of two simultaneous edits win',
    async () => {
  ...
  it(
    'waits out a writer that holds the lock, instead of failing',
    async () => {
      const { root, configDir, library } = setupLibrary(journalMode);
      library.close();
  ...
  it(
    'never lets two processes claim the same folder name',
    async () => {
      const { root, configDir, library } = setupLibrary(journalMode);
      library.close();
...
describe('migration skew', () => {
  it(
    'makes an older app open a newer library read-only rather than write to it',
    async () => {
...
describe('data_version change detection', () => {
  it('notices when another connection commits, and not when this one does', () => {
    ...
    try {
      ...
      expect(secondService.listItems().map((row) => row.name).sort()).toEqual(['Mine', 'Theirs']);
    } finally {
      second.close();
      first.close();
    }
